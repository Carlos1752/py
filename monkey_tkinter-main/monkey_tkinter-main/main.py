...
import pyperclip   #pyperclip整个插件文件夹需要放到此项目文件夹同目录


bundle_dir = getattr(sys, '_MEIPASS', path.abspath(path.dirname(__file__)))
path_to_dat = path.join(bundle_dir, 'img1.png')
...
run_test = ('--pct-touch', '--pct-motion', '--pct-trackball', '--pct-nav',
            '--pct-majornav', '--pct-syskeys', '--pct-appswitch', '--pct-anyevent',)
# 运行参数百分比配置
...
class MainPage:
    ...
    def main_page(self):
        ...
        # 间隔时间控件
        Label(root, text='间隔时间ms:').place(x=10, y=85)
        self.ms = Entry(root, bd=5, bg='white')
        self.ms.place(x=90, y=85)
        ...
        # 显示到页面
        opt_1.place(x=10, y=self.optionmenu_y + 85)
        opt_2.place(x=90, y=self.optionmenu_y + 85)
        opt_3.place(x=90, y=self.optionmenu_y + 110)

        # 添加和删除参数控件
        Button(root, text='+', takefocus=0, command=self.add_option_menu, bg='Gainsboro', width=2, height=0,
               font=('Helvetica', '10')).place(x=205, y=155)
        Button(root, text='-', takefocus=0, command=self.delete_option_menu, bg='Gainsboro', width=2, height=0,
               font=('Helvetica', '10')).place(x=235, y=155)
    ...
    def add_option_menu(self):
        ...
        if self.num < 6:
            ...
            # 显示到页面
            add_opt_1.place(x=10, y=self.optionmenu_y + 88)
            add_opt_2.place(x=90, y=self.optionmenu_y + 75)
            add_opt_3.place(x=90, y=self.optionmenu_y + 100)
        else:
            print('最多添加5个')
    ...
    def get_page_name():
        ...
        if get_page:
            ...
            tkinter.messagebox.showinfo("提示", '已复制,请粘贴到输入框内！'
                                                '\n包名：{}'.format(get_page))
        else:
    ...
    def entry_forms(self):
        ...
        for r, p in zip(self.run_option_menu_list, self.percentage_option_menu_list):
            run_data_list.append([r.get(), p.get()])
            # run_data_list.append([r.get(), r.sub('\D', '', p.get())])
